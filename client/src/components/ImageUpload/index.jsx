    ...
    return(
        <>  
            <div className="input-content">
                ...
                <div className={`input-img-content ${isDragging ? "dragging" : ""}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}> {/* Pomocí isDragging přidávám styl divu aby při přetahování souboru změnil svojí barvu*/}
                    {imageUrl ? <img src={imageUrl} alt="Nahraný obrázek" /> : <p>Přetáhněte sem obrázek <br/>nebo klikněte na nahrát</p>}
                </div>
                <div className="upload-btn-box">
                    <label htmlFor={`file-upload-${imgId}`} id="file-upload-text">
                        <img src={uploadIcon} alt="" /> Nahrát
                    </label>
                    <input type="file" id={`file-upload-${imgId}`} className="file-upload" onChange={previewImage} />
                </div>
            </div>
        </>
    )
