...
exports.getAllUserContact = async (req, res) => {
    try {
        ...
        if(!contacts) return res.status(404).json({ message: "Kontakty nenalezeny" })
...
exports.getUserContactById = async (req, res) => {
    try{
        ...
        if(!contact) return res.status(404).json({ message: "Kontakt nebyl nalezen" })
        if(contact.user_id.toString() !== req.user.userId) return res.status(404).json({ message: "Kontakt nebyl nalezen" })
...
exports.updateContact = async (req, res) => {
    try{
        ...
        if(contact.user_id.toString() !== req.user.userId) return res.status(404).send({ message: "Kontakt nenalezen."})
